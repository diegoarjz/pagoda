...
struct interpreter_visitor
{
	...
	builtin_value visit(const ast::node_definition &node_definition)
	{
		...
		auto context =
		    std::make_shared<Context>(ParameterIdentifier::CreateIdentifier(node_definition.m_nodeType).second);
		...
		struct parameter_setter : public boost::static_visitor<void>
		{
			...
			void operator()(const float &v) { m_context->CreateParameter<FloatParameter>(m_parameterName, v); }
