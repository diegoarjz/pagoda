...
GraphDotExporter::GraphDotExporter(GraphPtr graph)
	: m_graph(graph),
	  m_rankBy(RankBy::None),
	  m_rankDirection(RankDirection::LeftToRight),
...
void GraphDotExporter::GraphDotExporter::SetRankBy(const RankBy &rankBy)
{
	m_rankBy = rankBy;
}

RankBy GraphDotExporter::GetRankBy() const
{
	return m_rankBy;
}

void GraphDotExporter::SetRankDirection(const RankDirection &rankDirection)
{
	m_rankDirection = rankDirection;
}

RankDirection GraphDotExporter::GetRankDirection() const
{
	return m_rankDirection;
}

void GraphDotExporter::SetShowParameters(bool &show)
{
	m_showParameters = show;
}

bool GraphDotExporter::GetShowParameters() const
{
...
void GraphDotExporter::Export(std::ostream &outStream)
{
	...
	for (auto n : nodes)
	{
		...
		if (m_showParameters)
		{
			auto parameters = node->GetParameterContext()->GetParameters();
...
std::vector<NodeExportInfo> GraphDotExporter::GetNodes()
{
	...
	switch(m_rankBy)
	{
		case RankBy::None:
		{
			...
			std::transform(allNodes.begin(), allNodes.end(), std::back_inserter(nodes),
				[] (const NodePtr &n) { return NodeExportInfo{n, 0}; }):
			break;
		}
		case RankBy::Depth:
		{
			std::unordered_map<NodePtr, uint32_t> depths;
			auto delegate = [&depths, &m_graph](const NodePtr &n)
			{
			...
			std::transform(depths.begin(), depths.end(), std::back_inserter(nodes),
				[&depths](const NodePtr &n) { return NodeExportInfo{n, depths[n]}; });
