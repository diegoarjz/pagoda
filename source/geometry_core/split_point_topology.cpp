...
SplitPointTopology::SplitPointHandle SplitPointTopology::GetSplitPoint(const PointHandle &p, const EdgeHandle &e) const;

SplitPointTopology::FaceHandle SplitPointTopology::GetFace(const PointHandle &p, const EdgeHandle &e) const;

SplitPointTopology::PointHandle SplitPointTopology::GetPoint(const SplitPointHandle &s) const
{
	return SplitPointHandle(m_splitPoints.Get(s.GetIndex()).m_splitPoint);
}
...
{
	return EdgeHandle(GetOutEdge(GetSplitPoint(f.GetIndex())));
}
...
SplitPointTopology::SplitPointHandle SplitPointTopology::GetPrevSplitPoint(const SplitPointHandle &s) const
{
	...
	return SplitPointTopology(prevEdge.m_source);
}
...
SplitPointTopology::CreateFaceResult SplitPointTopology::CreateFace()
{
	...
	// SplitPoints
	IndexSplitPointPairArray<3> splitPoints = {
		m_points.CreateAndGet(),
		m_points.CreateAndGet(),
		m_points.CreateAndGet()
	};
	...
	// Edges
	IndexEdgePairArray<3> edges = {
		m_edges.CreateAndGet(),
		m_edges.CreateAndGet(),
		m_edges.CreateAndGet()
	};
...
SplitPointTopology::CreateFaceResult SplitPointTopology::CreateFace(const PointHandle &p1, const PointHandle &p1, const PointHandle &p2)
{
	// Points
	IndexPointPairArray<3> points = {
		p0,
...
SplitPointTopology::CreateFaceResult SplitPointTopology::CreateFace(const IndexPointPairArray<3> &points,
																	const IndexSplitPointPairArray<3> &splitPoints,
																	const IndexEdgePairArray<3> &edges)
{
	CreateFaceResult result;
	
	FaceContainer_t::IndexValuePair_t face = m_faces.CreateAndGet();
	face.m_splitPoint = std::get<0>(splitPoints[0]);
	...
	for (uint32_t i = 0; i < 3; ++i)
	{
		...
		sourceSplitPoint.m_outgoingEdge = std::get<0>(edge[i]);
		sourceSplitPoint.m_incomingEdge = std::get<0>(edge[(i-1) % 3]);
	...
	result.m_face = FaceHandle(std::get<0>(face));
	for (auto i = 0u; i < 3; ++i)
	{
		result.m_points[i] = PointHandle(std::get<0>(points[i]);
	}
...
SplitPointTopology::SplitPointHandle SplitPointTopology::AddPointToFace(const FaceHandle &f, const EdgeHandle &e, const PointHandle &p)
{
	...
	newSplitPoint.m_point = p.GetIndex();
	newSplitPoint.m_face = f.GetIndex();
	newSplitPoint.m_incomingEdge = e.GetIndex();
	newSplitPoint.m_outgoingEdge = std::get<0>(newEdge);
	
	newEdge.m_source = std::get<0>(newSplitPoint);
	newEdge.m_destination = edge.m_destination;
...
SplitPointTopology::SplitPointHandle SplitPointTopology::SplitEdge(const EdgeHandle &e, const PointHandle &p)
{
	...
	point.m_edges.insert(newEdge); // TODO: check if the edge was inserted.
	
	newSplitPoint.m_point = p.GetIndex();
	newSplitPoint.m_incomingEdge = e.GetIndex();
	newSplitPoint.m_outgoingEdge = std::get<0>(newEdge);
	newSplitPoint.m_face = GetFace(e).GetIndex();
	...
	newSplitPoint.m_incomingEdge = std::get<0>(newEdge);
...
{
	Edge& edge = m_deges.Get(e.GetIndex());
	...
	prevPoint.m_splitPoints.erase(edge.m_source);
...
SplitPointTopology::SplitFaceResult SplitPointTopology::SplitFace(const FaceHandle &f, const EdgeHandle &e0, const EdgeHandle &e1)
{
...
void SplitPointTopology::DeleteFace(const FaceHandle &f)
{
	...
	SplitPoint &splitPoint = m_splitPoints.Get(f.m_splitPoint);
	...
	while (currentSplitPointIndex != face.m_splitPoint)
	{
		...
		point.m_edges.remove(currentSplitPointIndex);
...
SplitPointTopology::EdgeHandleSet SplitPointTopology::GetEdges(const PointHandle &p0, const PointHandle &p1)
{
	...
	std::unordered_set<Index_t> edgesToCheck = m_point0.m_edges + m_point1.m_edges;
	
	for (const auto& edgeIndex : edgesToCheck)
	{
		const Index_t sourcePointIndex = GetPoint(GetSource(edgeIndex)).GetIndex();
		const Index_t destinationPointIndex = GetPoint(GetDestination(edgeIndex)).GetIndex();
...
SplitPointTopology::FaceHandleSet SplitPointTopology::GetFaces(const PointHandle &p0, const PointHandle &p1)
{
	...
	{
		const auto &sourceSplitPoint = m_splitPoints.Get(GetSourcePoint(e).GetIndex());
...
bool SplitPointTopology::ValidateSplitPoint(const Index_t &s)
{
	...
	bool isValid = true;
	const SplitPoint &splitPoint = m_splitPoint(s);
	...
	{
		LOG_ERROR("Incoming Edge %d of Split Point %d references Split Point %d", splitPoint.m_incomingEdge, s, incomingEdge.m_destination);
		isValid = false;
	}
	if (outgoingEdge.m_source != s)
	{
		LOG_ERROR("Outgoing Edge %d of Split Point %d references Split Point %d", splitPoint.m_outgoingEdge, s, outgoingEdge.m_source);
		isValid = false;
	}
...
bool SplitPointTopology::ValidateEdge(const Index_t &e)
{
	...
	{
		LOG_ERROR("Source Split Point %d doesn't reference Edge %d (references %d)", edge.m_source, e, source.m_outgoingEdge);
		isValid = false;
	}
	if (destination.m_incomingEdge != e)
	{
		LOG_ERROR("Destination Split Point %d doesn't reference Edge %d (references %d)", edge.m_destination, e, destination.m_incomingEdge);
		isValid = false;
	}
...
bool SplitPointTopology::ValidateFace(const Index_t &f)
{
	...
	{
		LOG_ERROR("Split Point %d for Face %d doesn't exist", 
	}
	...
	while (currentSplitPointIndex != face.m_splitPoint)
	{
		...
		{
			LOG_ERROR("Split Point %d doesn't reference Face %d (references %d)", currentSplitPointIndex, f, splitPoint.m_face);
			isValid = false;
		}
