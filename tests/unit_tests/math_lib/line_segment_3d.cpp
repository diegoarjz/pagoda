...
TEST(LineSegment3D, when_using_default_constructor_should_create_a_unit_line_segment_in_the_x_axis)
{
	...
	ASSERT_TRUE(l.GetSourcePoint() == (boost::qvm::vec<float, 3>{0, 0, 0}));
	ASSERT_TRUE(l.GetTargetPoint() == (boost::qvm::vec<float, 3>{1, 0, 0}));
}

TEST(LineSegment3D, when_using_two_points_constructor_should_create_the_respective_line_segment)
{
	LineSegment3D<float> l(boost::qvm::vec<float, 3>{0, 0, 0}, boost::qvm::vec<float, 3>{1, 2, 3});
	ASSERT_TRUE(l.GetSourcePoint() == (boost::qvm::vec<float, 3>{0, 0, 0}));
	ASSERT_TRUE(l.GetTargetPoint() == (boost::qvm::vec<float, 3>{1, 2, 3}));
}
...
{
	LineSegment3D<float> l1(boost::qvm::vec<float, 3>{0, 0, 0}, boost::qvm::vec<float, 3>{1, 2, 3});
	LineSegment3D<float> l2(boost::qvm::vec<float, 3>{0, 0, 0}, boost::qvm::vec<float, 3>{1, 2, 4});
	ASSERT_FALSE(l1 == l2);
	ASSERT_TRUE(l1 != l2);
}
