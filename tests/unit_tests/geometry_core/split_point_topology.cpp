...
TEST_F(SplitPointTopologyCreateFaceTest, when_creating_faces_should_be_able_to_reuse_points)
{
	...
	auto results2 = m_topology.CreateFace(m_topology.GetPoint(results1.m_splitPoints[0]),
	                                      m_topology.GetPoint(results1.m_splitPoints[1]));

	EXPECT_EQ(m_topology.GetPoint(results2.m_splitPoints[0].GetIndex()),
	          m_topology.GetPoint(results1.m_splitPoints[0].GetIndex()));
	EXPECT_EQ(m_topology.GetPoint(results2.m_splitPoints[1].GetIndex()),
	          m_topology.GetPoint(results1.m_splitPoints[1].GetIndex()));
