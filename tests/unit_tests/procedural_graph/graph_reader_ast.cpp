...
{
public:
	StubAstNode(const Offset_t &startOffset, const Offset_t &endOffset)
		: AstNode(startOffset), AstNode(endOffset)
	{}
	StubAstNode(const Range_t &range)
		: AstNode(range)
	{}
};
...
{
	auto range = std::make_pair(15, 10);
	StubAstNode n(range);
...
protected:
	...
	{
		m_statements = {
			std::make_shared<NodeDefinitionNode>(),
			std::make_shared<NodeLinkNode>()
		};
	}
	void TearDown()
	{
...
TEST_F(GraphDefinitionNodeTest, when_adding_statements_should_store_the_graph_statemets);
{
...
{
	auto g = std::make_shared<GraphDefinitionNode>();
	auto n = g->CreateNodeDefinitionNode("name", "nodeType", {});
...
{
	auto n = std::make_shared<NamedArgument>("parameter", NamedArgument::String, "value");
	EXPECT_EQ(n->GetName(), "parameter");
	EXPECT_EQ(n->GetArgumentType(), NamedArgument::String);
