...
TEST_F(NodeDefinitionNodeTest, when_constructing_with_offsets_name_node_type_and_construction_arguments_should_store_them)
{
...
{
	auto n = std::make_shared<NodeDefinitionNode>(std::make_pair(std::size_t(0), std::size_t(1)), "nodeName", "Operation", constructionArgs);
	EXPECT_EQ(n->GetNodeName(), "nodeName");
	EXPECT_EQ(n->GetNodeType(), "Operation");
	expectEqualArgs(n->GetConstructionArguments(), constructionArgs);
	EXPECT_EQ(n->GetExecutionArguments().size(), 0);
...
{
	auto n = std::make_shared<NodeDefinitionNode>(0,0, "n", "o", {});
...
{
	auto n = std::make_shared<NodeDefinitionNode>(0,0, "n", "0", {});
...
TEST_F(NodeDefinitionNodeTest, when_creating_an_execution_arg_should_store_it)
{
	...
	expectEqualArgs(n->GetExecutionArgument(), {a});
}
 ...
 * ***************/
 ...
 {
 	auto n = std::make_shared<NodeLinkNode>(0,1, {});
 ...
 TEST_F(NodeLinkedNodeTest, when_iterating_over_should_access_the_linked_nodes_in_order)
 {
 	auto n = std::make_shared<NodeLinkNode>(0,1, m_linkedNodes);
 	NodeLinkedNode::NodeLinkContainer_t c(std::begin(n), std::end(n)); // does the same as iterating
