...
class Adjacent : public Query
{
	...
	Adjacent(Graph &graph, QueryHandle_t queryHandle,
	         std::shared_ptr<Query> &&query = nullptr, int32_t direction = 0);
	Adjacent(Graph &graph, NodeSet &nodeSet,
	         std::shared_ptr<Query> &&query = nullptr, int32_t direction = 0);

	~Adjacent() override;

	void AppendToString(std::stringstream &os,
