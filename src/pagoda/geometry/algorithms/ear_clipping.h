...
private:
	...
	{
		VertexLinkedListNode(const Index_t splitPoint, const boost::qvm::vec<float, 3> &position)
...
public:
	...
	void Execute(GeometryPtr geometryIn, GeometryPtr geometryOut)
	{
		...
		for (auto faceIter = geometryIn->FacesBegin(); faceIter != geometryIn->FacesEnd(); ++faceIter)
		{
			...
			boost::qvm::vec<float, 3> faceNormal = geometryIn->GetFaceAttributes(*faceIter).m_normal;
			...
			{
				boost::qvm::vec<float, 3> curr = geometryIn->GetPosition(geometryIn->GetPoint(*fspIter));
			...
			for (auto i = 0u; i < vertexLinkedList.size(); ++i)
			{
				...
				boost::qvm::vec<float, 3> prev = vertexLinkedList[i].m_prev->m_position;
				boost::qvm::vec<float, 3> curr = vertexLinkedList[i].m_position;
				boost::qvm::vec<float, 3> next = vertexLinkedList[i].m_next->m_position;
			...
			while (!earTips.empty() && trianglesFound < vertexLinkedList.size() - 2)
			{
				...
				for (const auto &adj : {prev, next})
				{
					...
					boost::qvm::vec<float, 3> prevPos = adj->m_prev->m_position;
					boost::qvm::vec<float, 3> currPos = adj->m_position;
					boost::qvm::vec<float, 3> nextPos = adj->m_next->m_position;
...
private:
	...
	{
		boost::qvm::vec<float, 3> p0 = v.m_prev->m_position;
		boost::qvm::vec<float, 3> p1 = v.m_position;
		boost::qvm::vec<float, 3> p2 = v.m_next->m_position;
	...
	bool pointInsideTriangle(const boost::qvm::vec<float, 3> &P, const boost::qvm::vec<float, 3> &a, const boost::qvm::vec<float, 3> &b, const boost::qvm::vec<float, 3> &c)
	{
		boost::qvm::vec<float, 3> e0 = c - a;
		boost::qvm::vec<float, 3> e1 = b - a;
		boost::qvm::vec<float, 3> e2 = P - a;
