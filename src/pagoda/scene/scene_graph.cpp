...
SceneNodePtr SceneGraph::GetNodeAtPath(const Path& path) const {
  auto currNode = GetRootNode();
  const auto numComponents = path.GetNumComponents();

  for (std::size_t i = 0; i < numComponents; ++i) {
    if (currNode == nullptr) {
      break;
    }
    const auto component = path.GetComponent(i);
    currNode = currNode->GetChild(component);
  }
